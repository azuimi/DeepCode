...
REPO="${DEEPCODE_REPO:-https://github.com/azuimi/DeepCode.git}"
INSTALL_DIR="${DEEPCODE_INSTALL_DIR:-$HOME/.deepcode-agent}"
BRANCH="${DEEPCODE_BRANCH:-main}"

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'
BOLD='\033[1m'; RESET='\033[0m'
info()    { printf "${CYAN}[deepcode]${RESET} %s\n" "$*"; }
success() { printf "${GREEN}[deepcode]${RESET} ${BOLD}%s${RESET}\n" "$*"; }
warn()    { printf "${YELLOW}[deepcode]${RESET} %s\n" "$*" >&2; }
die()     { printf "${RED}[deepcode] ERROR:${RESET} %s\n" "$*" >&2; exit 1; }
...
main() {
    printf "\n${BOLD}╔══════════════════════════════════════════╗${RESET}\n"
    printf   "${BOLD}║        deepcode  一键安装                    ║${RESET}\n"
    printf   "${BOLD}╚══════════════════════════════════════════╝${RESET}\n\n"
  ...
  安装方法：sudo apt install python3.11   (Debian/Ubuntu)
    ...
    if [[ -d "$INSTALL_DIR/.git" ]]; then
        ...
        info "克隆 deepcode 到 ${INSTALL_DIR} ..."
        rm -rf "$INSTALL_DIR"
    ...
    mkdir -p "$BIN_DIR"
    LAUNCHER="$BIN_DIR/deepcode"

    cat > "$LAUNCHER" <<EOF
#!/usr/bin/env bash
exec "${VENV_DIR}/bin/deepcode" "\$@"
EOF
    chmod +x "$LAUNCHER"

    printf "\n"
    success "deepcode 安装完成！"
    printf "\n"
