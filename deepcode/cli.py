...
这个模块负责把“用户怎么启动 deepcode”翻译成 runtime 能理解的对象：
...
SECRET_ENV_NAMES_VAR = "DEEPCODE_SECRET_ENV_NAMES"
...
def build_agent(args):
    """根据 CLI 参数装配出一个可运行的 DeepCode 实例。
    ...
    - 输出：一个新的 `DeepCode`，或一个从旧 session 恢复出来的 `DeepCode`
    ...
    store = SessionStore(workspace.repo_root + "/.deepcode/sessions")
...
def build_arg_parser():
    ...
    parser.add_argument(
        "--config", default=None, help="Path to a DeepCode TOML config file."
    )
    parser.add_argument(
        "--provider",
        default=None,
    ...
    parser.add_argument(
        "--memory-dir",
        default=None,
        help="Memory directory. Defaults to .deepcode/memory in the workspace.",
    )
    parser.add_argument(
        "--no-auto-dream",
        action="store_true",
...
def main(argv=None):
    ...
    if mode == "tui":
        from .tui.app import DeepCodeTuiApp
