...
DeepCode owns session state, workspace context, memory, checkpoints, and persistence.
...
class DeepCode(RuntimeSecretsMixin, RuntimeCheckpointsMixin):
    def __init__(
        self,
        model_client,
        workspace,
        ...
        self.run_store = run_store or RunStore(
            Path(workspace.repo_root) / ".deepcode" / "runs"
        )
    ...
    def _resolve_memory_dir(self, memory_dir):
        if memory_dir:
            ...
            path = self.root / ".deepcode" / "memory"
    ...
    def build_prefix(self):
        ...
        text = textwrap.dedent(
            f"""\
            You are deepcode, a small local coding agent working inside a local repository.
