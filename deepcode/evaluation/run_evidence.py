"""Read-only helpers for DeepCode run/session evidence.

These helpers intentionally do not import or instantiate the DeepCode runtime. They
only interpret files that a completed CLI/REPL/TUI run left under `.deepcode/`.
"""
...
class RunEvidence:
    ...
    def latest(cls, workspace: Path) -> "RunEvidence":
        ...
        run_dir = _latest_dir(workspace / ".deepcode" / "runs")
        ...
        session_path = _latest_file(workspace / ".deepcode" / "sessions", "*.json")
        session_event_path = _latest_file(workspace / ".deepcode" / "sessions", "*.events.jsonl")
        return cls(
            workspace=workspace,
            run_dir=run_dir,
