...
You are performing a dream: a reflective pass over DeepCode's memory files. Synthesize recent signal into durable, well-organized memory files so future sessions can orient quickly.
...
def run_dream(agent, quiet=False, session_ids=None):
    from ..core.runtime import DeepCode
    ...
    try:
        ...
        memory_scope = Path(".deepcode") / "memory"
    dream_agent = DeepCode(
...
def maintain_memory_after_turn(agent, final_answer):
    ...
    thread = threading.Thread(target=_background_dream, name="deepcode-auto-dream", daemon=True)
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".deepcode" / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".deepcode" / "memory")
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".deepcode" / "memory") if workspace_root is not None else None
