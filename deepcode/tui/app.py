...
class DeepCodeTuiApp(App):
    """Textual shell for the existing DeepCode runtime.

    The TUI is deliberately a presentation layer: CLI argument parsing and agent
    construction still live in `deepcode.cli`, while turns are driven through the
    same `Engine.run_turn()` generator that powers the plain REPL.
    """

    CSS = DEEPCODE_TUI_CSS
    BINDINGS = [
