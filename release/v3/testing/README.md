...
这个目录把 DeepCode v3 相对 `main` 的真人使用场景测试整理成一个可以复盘、重跑、继续扩展的测试包。

它不是单元测试说明，也不是 pytest 清单。它记录的是：像真实用户一样从 `uv run deepcode`、`--repl`、slash command、resume、provider profile、skills、worker、memory 等入口使用 DeepCode，然后只读检查 `.deepcode/runs` 和 `.deepcode/sessions` 产物。
...
| `deepcode/evaluation/run_evidence.py` | 从真实 `.deepcode/runs`、`.deepcode/sessions` 读取证据 |
...
- 场景必须从用户入口驱动 DeepCode，不能 import `DeepCode` 直接调 runtime。
- 验证器可以读文件，但只能读 DeepCode 自己写出的 artifacts 和 scenario workspace。
- 输出目录必须在 repo 外，避免 DeepCode 向上发现真实 repo root。
- live provider 默认用 DeepSeek，配置来自项目 `.deepcode.toml`，不把 key 写进文档或产物。
