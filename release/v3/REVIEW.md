# DeepCode v3 review pack

## Project pitch

DeepCode is a small local coding agent that turns one user request into a bounded
...
- `DeepCode` owns session state, memory, tools, and workspace safety.
...
be validated through persisted `.deepcode/sessions/*.json` and
`.deepcode/sessions/*.events.jsonl` artifacts before treating a runtime change as
done.
...
- `.deepcode/runs/<run_id>/task_state.json`
- `.deepcode/runs/<run_id>/trace.jsonl`
- `.deepcode/runs/<run_id>/report.json`
