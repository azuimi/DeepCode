...
DeepCode 跑在本地仓库里，接上一个模型 provider，就能读代码、跑命令、改文件、
保留运行证据，并把有价值的上下文沉淀成本地记忆。

</div>

<p align="center">
  <img src="assets/screenshots/deepcode-tui-intro.png" alt="deepcode TUI 启动界面" width="960">
</p>
...
DeepCode 是一个本地终端里的 coding agent，运行在你的仓库上下文里。一次 agent 运行会被拆成几个可观察的部分：
...
- **session / run evidence**：对话、事件流、trace、report 都写到本地 `.deepcode/`。
- **memory / dream**：把 daily log 整理成长期 topic，下次 session 可以继续用。

deepcode 关注本地 coding agent 的工程边界：配置清楚、任务能续接、结果能复盘。
...
| --- | --- |
| ![deepcode TUI 工具表](assets/screenshots/deepcode-tui-tools.png) | ![deepcode TUI skills 和 help](assets/screenshots/deepcode-tui-skills-help.png) |

| Memory 和 durable topics | Slash command 工作区 |
| --- | --- |
| ![deepcode TUI memory 和 skills](assets/screenshots/deepcode-tui-memory-skills.png) | ![deepcode TUI slash command 补全](assets/screenshots/deepcode-tui-latest.png) |
...
```bash
curl -fsSL https://raw.githubusercontent.com/azuimi/DeepCode/main/install.sh | bash
```

源码安装：

```bash
git clone https://github.com/azuimi/DeepCode.git
cd deepcode
pip install -e .
```
...
deepcode 启动前先解析一个 **provider profile**。一个 profile 主要由四项组成：
...
```text
CLI 参数 > 环境变量 > 项目 .deepcode.toml > 全局 ~/.config/deepcode/config.toml > 代码默认值
```

### 方式一：项目 `.deepcode.toml`

这是最推荐的配置方式，适合每个仓库独立指定 provider：

```bash
cp .deepcode.toml.example .deepcode.toml
$EDITOR .deepcode.toml
```

`.deepcode.toml` 默认被 `.gitignore` 忽略，不要把真实 key 提交进 git。
...
```bash
export DEEPCODE_API_KEY=sk-...
export DEEPCODE_BASE_URL=https://api.openai.com/v1
export DEEPCODE_MODEL=gpt-5.4
```
...
```bash
deepcode --provider openai --model gpt-5.4 --base-url https://api.openai.com/v1
deepcode --provider deepseek --approval ask --max-steps 80
deepcode --config /path/to/custom.toml --cwd /path/to/repo
```
...
```bash
deepcode                              # 默认 Textual TUI
deepcode --repl                       # 普通终端 REPL
deepcode "找出测试失败的根因"          # one-shot 任务
deepcode --resume latest              # 续接最近 session
deepcode --cwd /path/to/repo          # 指定工作目录
```

常用运行参数：

```bash
deepcode --approval ask               # shell / 写文件前询问
deepcode --approval auto              # 普通操作自动通过
deepcode --approval never             # 非交互模式
deepcode --sandbox best_effort        # 尽量隔离 shell 命令
deepcode --no-auto-dream              # 关闭后台 memory 整合
```
...
| --- | --- |
| 项目配置 | `.deepcode.toml` |
| 全局配置 | `~/.config/deepcode/config.toml` |
| 会话历史 | `.deepcode/sessions/<id>.json` |
| 事件流 | `.deepcode/sessions/<id>.events.jsonl` |
| 运行证据 | `.deepcode/runs/<run_id>/` |
| 记忆索引 | `.deepcode/memory/MEMORY.md` |
| Daily logs | `.deepcode/memory/logs/YYYY/MM/YYYY-MM-DD.md` |
| Durable topics | `.deepcode/memory/topics/*.md` |
| 用户 skills | `~/.deepcode/skills/<name>/SKILL.md` |
| 项目 skills | `skills/<name>/SKILL.md` 或 `.deepcode/skills/<name>/SKILL.md` |
...
DEEPCODE_LIVE_SMOKE=1 pytest tests/test_release_smoke.py -q
```
...
| 入口 | 内容 |
| --- | --- |
| [配置](docs/configuration.md) | provider profile、`.deepcode.toml`、环境变量和 sandbox 配置。 |
